...
//  MilkyWay
//
//  Created by Aluno on 25/05/2018.
...
class SettingsTableViewController: UITableViewController {
    ...
    func updateUI(_ producer: Producer) {
        nameTextField.text = producer.name
        addressTextView.text = producer.address
    }

    func loadProducer() {
        if let savedProducer = Producer.loadProducer() {
            updateUI(savedProducer)
        } else {
            updateUI(Producer.loadSampleProducer())
        }
